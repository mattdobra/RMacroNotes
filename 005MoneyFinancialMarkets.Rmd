...
**Deflation** is the inverse of inflation, and happens when the general price level within the economy is decreasing.

**Disinflation** is when the inflation rate falls, but the rate of inflation is still positive.
...
The media and politicians are at best not very good, and at worst incredibly dishonest, when it comes to understanding the difference between *inflation*, *deflation*, and *disinflation.*

For example, Jeremy Hunt (UK Minister of the Exchequer) made this statement in July 2023. This is in response to the UK inflation rate having fallen to 7.9% from its high of 11.1% in October 2022.

What have this journalist and this politician gotten wrong here?

Why might they have gotten it wrong?
:::

::: {style="float:left; width:40%;"}
```{r out.width = "90%", fig.align = 'left'}
knitr::include_graphics("images/uk_inflation.jpg")
```
:::
...
Joe Biden got ratio'd for expressing a similar sentiment in November, 2023.

The US inflation rate hit a high of 9.1% in June 2022 but had been in the 3%-3.5% range since June 2023.
:::
...
```{r out.width = "90%", fig.align = 'left'}
knitr::include_graphics("images/bidenflation.png")
```
:::
...
The Consumer Price index is the most commonly used measure in the News.
...
When trying to control inflation, the Federal Reserve prefers to use the PCE/Core PCE

They prefer PCE because it:

-   Includes urban *and* rural consumers
-   Includes money spent on behalf of consumers in addition to personal spending
-   Allows for substitution effects when prices are volatile

Core PCE takes out the most volatile elements of PCE, food and energy.

## Inflation

::: {style="float:right; width:70%;"}
```{r get_CPI, include = FALSE}
getSymbols("CPIAUCNS", src = "FRED", return.class = "data.frame") # Inflation Rate
cpi <- CPIAUCNS
rm(CPIAUCNS)
...
inflation %>% 
    ...
    scale_y_continuous(labels = scales::percent_format(accuracy = 2),
                       # expand = c(.04,.04),
                       breaks = seq(-.2,3, by = .02),
                       limits = c(min(inflation$inflation[inflation$date > as.Date("2018-01-01")]) - 0.01, 
           max(inflation$inflation[inflation$date > as.Date("2018-01-01")]) + 0.01)) +
...
::: {style="float:right; width:70%;"}
```{r get_PCE, include = FALSE}
getSymbols("PCEPI", src = "FRED", return.class = "data.frame") # U3 Unemployment Rate
pce <- PCEPI
rm(PCEPI)
pce <- rename(pce, pce = PCEPI)
pce <- rownames_to_column(pce, "date")
pce$date <- as.Date(pce$date)

curpce <- pce[length(pce$pce), ]
pce20 <- pce[length(pce$pce)-12, ]


lubridate::month(as.Date(curcpi$date[1]), label = TRUE, abbr = FALSE)

inflation_pce <- pce %>% 
    mutate(inflation_pce = (pce-lag(pce,12))/lag(pce,12))

curinflation_pce <- inflation_pce[length(inflation_pce$inflation_pce), ]
...
inflation %>% 
    ...
    geom_line(data = inflation_pce, 
              aes(x = date, y = inflation_pce), 
              color = color3, 
              lty = "dashed", 
              size = 1) +
    geom_rect(data = recession.df, aes(xmin = start, xmax = end, ymin = -Inf, ymax = +Inf), fill = color5, alpha = .3) +
...
#                       expand = c(.04,.04),
    ...
    labs(
        ...
        subtitle = "Difference between CPI and PCE in annual inflation rates",
        x = "Date",
        y = "Inflation Rate",
        caption = "Data from FRED, created by Matt Dobra @mattdobra"
    ) 
...
PCE inflation is generally a bit lower than CPI inflation:

-   Current CPI inflation is `r round(100*curinflation$inflation,1)`%
-   Current PCE inflation is `r round(100*curinflation_pce$inflation_pce, 1)`%.
:::
...
In the 2021 inflation, politicians and political pundits argued that a variety of microeconomic forces were the cause of inflation.
...
Blaming inflation on corporate greed (greedflation) is the worst of the bunch.

You can't explain a variable with a constant!

Were corporations *less* greedy when inflation was lower? Was the 2023 decrease in inflation caused by a decrease in greed?

It *is* true that profits rise during an inflationary period, but the rising profit is the *effect* of inflation, not its *cause*!
...
**Money Illusion** is the notion that people think in nominal terms, not real terms.
...
The Volcker Disinflation provides a good example.
...
The Federal Reserve performs a number of functions. They:
...
MB is perfectly liquid since it *is* cash, M1 is very liquid since it includes immediately accessible deposits, and M2 is pretty liquid as well since it is money that people could access very quickly if they chose.
...
```{r get_FEDFUNDS, include = FALSE}
getSymbols("FEDFUNDS", src = "FRED", return.class = "data.frame") # U3 Unemployment Rate
fedfunds <- FEDFUNDS
rm(FEDFUNDS)
fedfunds <- rename(fedfunds, fedfunds = FEDFUNDS)
fedfunds <- rownames_to_column(fedfunds, "date")
fedfunds$date <- as.Date(fedfunds$date)

curfedfunds <- fedfunds[length(fedfunds$fedfunds), ]

lubridate::month(as.Date(curfedfunds$date[1]), label = TRUE, abbr = FALSE)
...
ggplot() +
    geom_line(data = fedfunds, aes(x = date, y = fedfunds/100), color = color2, size = 1.2) + 
    geom_rect(data = recession.df, aes(xmin = start, xmax = end, ymin = -Inf, ymax = +Inf), fill = color5, alpha = .3) +
    geom_hline(yintercept = 0, color = color3) +
     theme_classic()+
    scale_y_continuous(labels = scales::percent_format(accuracy = 2),
#                       breaks = seq(-.2,3, by = .05),
                       limits = c(NA,NA),
                       expand = c(0,.02)) +
    scale_x_date(limits = as.Date(c("1955-01-01", Sys.Date())),
        expand = c(0,0),   
        labels = date_format("%Y"),
        breaks = function(x)
            seq.Date(
                from = as.Date("1955-01-01"),
                to = as.Date(Sys.Date()),
                by = "10 years"
            )) +
    labs(
        title = "Federal Funds Rate",
        subtitle = "The US Federal Funds Rate since 1955",
        x = "Date",
...
::: {style="float:left; width:30%;"}
The main interest rate the Fed controls is the **Federal Funds Rate** - the rate banks charge each other on overnight loans.

The Federal Funds market came into existence in the 1920s, but the Fed only stareted collecting data on it mid-1954.
:::
...
fedfunds %>% 
    ...
    ggplot() +
    geom_line(aes(x = date, y = fedfunds/100), color = color2, size = 1.2) + 
    geom_rect(data = recession.df, aes(xmin = start, xmax = end, ymin = -Inf, ymax = +Inf), fill = color5, alpha = .3) +
    geom_hline(yintercept = 0, color = color3) +
     theme_classic()+
    scale_y_continuous(labels = scales::percent_format(accuracy = 2),
                       ...
                       breaks = seq(-.2,3, by = .02),
                       limits = c(NA,NA)) +
    scale_x_date(limits = as.Date(c("1978-01-01", Sys.Date())),
        expand = c(0,0),   
        labels = date_format("%Y"),
        breaks = function(x)
            seq.Date(
    ...
    labs(
        title = "Federal Funds Rate",
        subtitle = "Federal Funds Rate since 1985",
        x = "Date",
        y = "Federal Funds Rate",
        caption = "Data from FRED, created by Matt Dobra @mattdobra"
    ) 
...
::: {style="float:left; width:30%;"}
Recall that the Federal Reserve worked to drastically cut inflation in the early 1980s as part of the **Volcker Disinflation**.

To do this, they raised interest rates considerably to rein in inflation.

The Fed usually reduces interest rates to stave off recessions.
:::

## Open Market Operations - Covid
...
fedfunds %>% 
    ...
    labs(
        title = "Federal Funds Rate",
        subtitle = "The Federal Reserve's Covid Response",
        x = "Date",
        y = "Federal Funds Rate",
        caption = "Data from FRED, created by Matt Dobra @mattdobra"
    ) 
...
::: {style="float:left; width:30%;"}
The Fed Funds rate surrounding Covid is illustrative of the Fed's general approach to monetary policy:

-   In response to the initial economic downturn, they cut rates to basically zero.
-   As the economy recovered, they raised interest rates to try to prevent inflation.
:::

## Open Market Operations - Covid

```{r merge_funds_inflation, include = FALSE}
fundsdat <- left_join(fedfunds, inflation, by = join_by(date == date))

```

::: {style="float:right; width:70%;"}
```{r warning = FALSE, out.width = "100%", out.extra='style="float:right; padding:1px"'}
colours <- c("Federal Funds" = color2, "Inflation" = color3)


fundsdat %>% 
    filter(date>=as.Date("2020-01-01")) %>% 
    ggplot() +
    geom_line(aes(x = date, y = fedfunds/100, color = "Federal Funds"), size = 1.2) + 
    geom_line(aes(x = date, y = inflation, color = "Inflation"), size = 1.2) + 
    geom_rect(data = recession.df, aes(xmin = start, xmax = end, ymin = -Inf, ymax = +Inf), fill = color5, alpha = .3) +
    geom_hline(yintercept = 0, color = color3) +
     theme_classic()+
    scale_y_continuous(labels = scales::percent_format(accuracy = 2),
    ...
    labs(
        title = "Federal Funds Rate and Inflation",
        subtitle = "The Federal Reserve Raised Rates in Response to Rising Inflation",
        x = "Date",
        y = "Federal Funds Rate, Inflation",
        color = "Legend",
        caption = "Data from FRED, created by Matt Dobra @mattdobra"
    ) +
    scale_color_manual(values = colours)  + theme(legend.position = c(0.2, 0.8))
```
:::

::: {style="float:left; width:30%;"}
The government's Covid response generated inflation.

The Fed was slow to respond (initially the Fed and government called the inflation "transitory.")

When it became clear that inflation wasn't transitory, the Fed started increasing rates.
:::
