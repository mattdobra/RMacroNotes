...
ggplot() +
  scale_y_continuous(expand = c(0,0), limits = c(0,8), breaks = c(0)) +
...
## LRAS = 3
  geom_path(data = lras3, mapping = aes(x = x, y = y), color = color2, linewidth = 1.2) +
...
```{r}
dat <- tibble(spend = 5, inf = -1:6, gdp = 6:-1)
...
Key idea: if prices do not adjust quickly in the short run, changes to Aggregate Demand can cause recessions.
...
```{r out.width = "100%"}
knitr::include_graphics("images/horpmag1.png")
```
...
```{r out.width = "100%"}
knitr::include_graphics("images/horpmag2.png")
```
...
```{r out.width = "100%"}
knitr::include_graphics("images/horpmag3.png")
```
...
```{r out.width = "100%"}
knitr::include_graphics("images/coronavirusstimulus.png")
```
