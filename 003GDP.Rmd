...
- Typically we talk about national GDP, but we can also talk about sub-national GDP (state GDP, county GDP, etc).
...
- An immigrant from Cameroon works as a cook in a restaurant in San Francisco. Is the money he earns considered part of the GDP of the U.S. or Cameroon?
...
usgdp <- usgdp %>% 
    ...
    mutate(growthqq = (1 + (gdp - lag(gdp, 1))/lag(gdp, 1))^4 - 1)
...
- Data revisions have led many economists to conclude that the recession actually began in late 2000.
...
- $Y = C + I + G + NX$
    ...
    - $G =$ The market value of government purchases (not including transfers!)
...
ggplot() +
    ...
    theme_classic()+
    labs(
        title = "US GDP and GDI Historically Track Closely",
        subtitle = "Quarterly data since 1947",
        x = "Year",
        y = "Billions of 2012 Dollars",
        caption = "Data from U.S. BEA, Retrieved from FRED
        @mattdobra"
    ) 
...
Coming out of the Covid recession, however, these two measures have diverged quite substantially!

This divergence is fueling, at least in part, some of the "are we in a recession" debate of 2022.

</div>
 
<div style = "float: right; width: 70%;">

```{r GDI_Short, warning = FALSE}
...
ggplot() +
    ...
    geom_rect(data = recession.df, aes(xmin = start, xmax = end, ymin = -Inf, ymax = +Inf), fill = color5, alpha = .3) +
    scale_x_date(
        limits = as.Date(c("2017-01-01", Sys.Date())),
        labels = date_format("%Y"),
        breaks = function(x)
            seq.Date(
        ...
        ylim(17000,max(gdpdata$gdi)+1000) +
    annotate(geom="text", 
             x=as.Date("2021", "%Y"),
             y=20800, 
             label="GDI",
    ...
    theme_classic()+
    labs(
        title = "US GDP and GDI Diverged Post-Covid",
        subtitle = "Quarterly data since 2017",
        x = "Year",
        y = "Billions of 2012 Dollars",
        caption = "Data from U.S. BEA, Retrieved from FRED
        @mattdobra"
...
What's causing the divergence? Hypotheses include:

* Selling of inventories drive income, but not production
* Investment being mismeasured as purchase of intermediate goods
* Noise generated by high inflation
...
- GDP doesn't count non-market production
...
- The share of women in the labor force has almost doubled since 1950
...
- Increase government interest rate risk: if interest rates rise quickly, the government will be in a very bad fiscal position.
...
tempdat <- maddison %>% 
    filter(year == 1 | year == 1500 | year == 1800 | year == 1870 | year == 1900 | year == 1950) %>% 
    group_by(year) %>% 
    slice(which.max(rgdpnapc)) %>% 
...
```{r}
tempdat %>% 
    kable(format = "html", table.attr = "class=\"rmdtable\"", col.names = c("Year", "Region", "Real GDP Per Capita")) %>% 
...
- Encourage education and protects against "brain drain"
