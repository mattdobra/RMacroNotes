...
ggplot(mapping = aes(x = q, y = p)) + 
    ## Base Graph Begin**
    geom_path(data = demand, color = color2, linewidth = 1.2) + 
    annotate("text", x=8.4, y=2, label = "D[1]", color = color2, size = 6, parse = TRUE) +
    geom_path(data = supply, color = color3, linewidth = 1.2) + 
...
ggplot(mapping = aes(x = q, y = p)) + 
    ...
    ## Down arrow
       annotate("segment", x = .4, xend = .4, y = 6.8, yend = 5.2,
             arrow = arrow(length = unit(1, "lines")), colour = color4, linewidth = 1) +    
    ##editing the x and y axes for the equilibria.  If dots aren't needed, still need to keep limit and expand##
    scale_x_continuous(expand = c(0,0), limit = c(0,10), breaks = c(3,5,7), labels = expression(Q[D],Q[1], Q[S])) + ## Set breaks = equilibrium Q
    scale_y_continuous(expand = c(0,0), limit = c(0,10), breaks = c(5,7), labels = expression(P[1], P[H])) + ## Set breaks = equilibrium Q
...
ggplot(mapping = aes(x = q, y = p)) + 
    ...
    ## Demand Right Shift ##
    geom_path(data = demandright, color = color2, linewidth = 1.2, linetype = "dashed") + 
    annotate("text", x=9.4, y=3, label = "D[2]", color = color2, size = 6, parse = TRUE) +
    annotate("segment", x = 3.2, xend = 3.8, y = 7.2, yend = 7.8,
             arrow = arrow(length = unit(.5, "lines")), colour = color4, linewidth = 1) +
    ...
    ## Arrows
       ...
       annotate("segment", y = .4, yend = .4, x = 5.2, xend = 5.8,
             arrow = arrow(length = unit(.51, "lines")), colour = color4, linewidth = 1) +    
    ## Dashed Lines for equilibria
    annotate("segment", x=0, xend = 5, y = 5, yend = 5, linetype = "dotted") + # horizontal line
    annotate("segment", x=5, xend = 5, y = 0, yend = 5, linetype = "dotted") + # vertical line
...
ggplot(mapping = aes(x = q, y = p)) + 
    ...
    ## End Base Graph ##
    ##Supply Right Shift ##
    geom_path(data = supplyright, color = color3, linewidth = 1.2, linetype = "dashed") + 
...
ggplot(mapping = aes(x = q, y = p)) + 
    ...
    ## Demand Left Shift ##
    geom_path(data = demandleft, color = color2, linewidth = 1.2, linetype = "dashed") + 
...
ggplot(mapping = aes(x = q, y = p)) + 
    ...
    ## End Base Graph ##
    ##Supply Left Shift ##
    geom_path(data = supplyleft, color = color3, linewidth = 1.2, linetype = "dashed") +
...
1. Start with a market in equilibrium.
...
ggplot(mapping = aes(x = q, y = p)) + 
  ## Base Graph Begin**
  geom_path(data = demand2, color = color2, linewidth = 1.2) + 
...
ggplot(mapping = aes(x = q, y = p)) + 
  ## Base Graph Begin**
  geom_path(data = supply2, color = color3, linewidth = 1.2) + 
...
Because price controls prevent prices from rationing the available supply, some other mechanism must take its place. Examples include:
...
ggplot(mapping = aes(x = q, y = p)) + 
  ...
  ## Leave .4 from axes and 0.2 from start/end point 
  annotate("segment", x = .4, xend = .4, y = 5.2, yend = 6.8,
           arrow = arrow(length = unit(.5, "lines")), colour = color4, linewidth = 1.5) +    

  ## Quantity Control Line and Label
  annotate("segment", x = 3, xend = 3, y = 0, yend = 8.7,
           colour = color3, linewidth = 1) +    
...
- Specialization creates the division of knowledge
...
ggplot(mapping = aes(x = q, y = p)) + 
  ...
  annotate("segment", x=5, xend = 5, y = 0, yend = 5, linetype = "dotted") + # vertical line
  annotate("segment", x=0, xend = 7, y = 3, yend = 3, linetype = "dotted") + # horizontal line
  annotate("segment", x=0, xend = 8, y = 4, yend = 4, color = color4, linewidth = 1.5) + # horizontal line
  annotate("segment", x=3, xend = 3, y = 0, yend = 3, linetype = "dotted") + # vertical line
  annotate("segment", x=7, xend = 7, y = 0, yend = 3, linetype = "dotted") # vertical line
...
- **Capital Account** = foreign direct investment (FDI) + portfolio investment + loans to US govt and businesses
...
data %>% 
    ...
    geom_line(color = color2, linewidth = .7) +
    geom_point(color = color2, size = 1.5) +
    theme_classic()+
    geom_line(aes(date,capital), color = color3, linewidth = .7) +
    geom_point(aes(date,capital), color = color3, size = 1.5) +
    geom_hline(aes(yintercept = 0), lty = "solid", color = color4, linewidth = 1.2) + 
    annotate(
        geom = "text",
        x = as.Date("2012-1-01"),
        y = 140,
...
labor %>% 
  ...
  ggplot(aes(x = gdp, y = labor)) +
    ...
    labs(title = "Prevalence of Child Labor",
         ...
         y = "Percentage of Working 7-14 Year Olds",
