...
d %>% 
    ...
    annotate(geom = "text", x = 47, y = 2.5, label = "Demand", color = color2) +
    theme_classic()
...
- Goods are **substitutes** if you generally choose between one ore the other.  
...
```{r}
subcom <- tibble(Blank = c("Other Good Increases in Price", "Other Good Dncreases in Price"), Substitutes= c("Demand Shifts Right", "Demand Shifts Left"), Complements = c("Demand Shifts Left", "Demand Shifts Right"))
subcom %>% 
...
- You might simply decide that you like a product more (or less) than before--this would chane your demand for tat good.
- You could discover a new product that you didn't know existed.

## Tastes and Preferences

The increasingly common calling for boycotts of various companie (for any number of reasons) can radically alter preferences.
...
- **Quantity Controls**: **Price Floors** and **Price Ceilings**
- **Quantity Controls**
...
1. No; this simply means the us is a great place to invest, and foreign investment in the US leads ot more economic growth.
...
labor <- tibble(WDI(country = "all", indicator = c("labor" = "SL.TLF.0714.SW.TM", "gdp" = "NY.GDP.PCAP.PP.KD", "pop" = "SP.POP.TOTL"), start = 2005, end = curyear, extra = TRUE, cache = NULL))
...
```{r fig.width=6,fig.height=5, out.extra='style="float:right; padding:1px"'}

labor %>% 
  group_by(iso2c) %>% 
  slice(which.max(year)) %>% 
  mutate(continent = ifelse(region == "ZG" | country == "Egypt, Arab Rep." | country == "Algeria", "Africa", ifelse(region == "Z7", "Europe", ifelse(region == "8S" | region == "Z4" | region == "ZQ", "Asia", "Americas")))) %>% 
  arrange(desc(pop)) %>% 
  ggplot(aes(gdp, labor, color = continent)) +
  ...
  guides(size = FALSE) +
  labs(title = "Prevalence of Child Labor", subtitle = "Child labor is more intensive in countries with lower GDP per capita", y = "Average Weekly Hours of Child Labor", x = "GDP per Capita (2017 USD)", caption = "Data from World Bank WDI using most recent available data for each country") 
